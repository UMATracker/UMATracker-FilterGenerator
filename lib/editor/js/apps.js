...
App.getCodeFromSelectedBlock = function(){
    ...
    if(Blockly.Tooltip.lastSelectedBlockID!=null && block!=null){
	    block = block.outputConnection.parentBlock_;
    }
...
App.getBlockTypeFromSelectedBlock = function(){
    var block = Blockly.selected;
    if(block){
        data = block.type.split(/_(.+)?/);
        attributes = data[1].split(',');
        if (attributes[0] === '') {
            attributes = [];
        }
        return {'type': data[0], 'id': block.id, 'attributes': attributes};
    }
    return {'type': null, 'id': null, 'attributes': []};
}
