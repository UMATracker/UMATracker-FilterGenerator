...
Blockly.Blocks['polyRegionSelector_regionSelector'] = {
    init: function() {
        ...
        this.appendValueInput("NAME")
            ...
            .appendField(new Blockly.FieldTextInput("4"), "degree")
            .appendField("-gon")
            .appendField(array ,"array")
            .appendField(new Blockly.FieldCheckbox("TRUE"), "toggle");
...
function getPolygon(degree) {
    var delta = (2 * Math.PI) / degree;

    var array = [];

    for (var i=0; i<degree; i++) {
        var x = 0.4 * Math.cos(delta * i) + 0.5;
        var y = 0.4 * Math.sin(delta * i) + 0.5;

        array.push('[' + [x, y].toString() + ']');
    }

    return '[' + array.toString() + ']';
}

Blockly.Python['polyRegionSelector_regionSelector'] = function func(block) {
    if (typeof func.degree === 'undefined') {
        func.degree = 4;
    }

    var degree = parseInt(block.getFieldValue('degree'));
    if (func.degree != degree) {
        if (degree < 3) {
            degree = 3;
            block.setFieldValue(degree.toString(), 'degree')
        }

        func.degree = degree;
        var array = getPolygon(degree);
        block.setFieldValue(array, 'array');
    }
    ...
    var poly = 'np.array([[[x[0]*self.width, x[1]*self.height] for x in {0}]], dtype=np.int32)'.format(array);
    code.push('#cv2.fillPoly({0}, {1}, color=255)\r\n'.format(im_mask_name, poly));
